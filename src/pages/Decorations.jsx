...
export default function Decorations() {
        ...
        <div style={{ padding: 18 }}>
          ...
          <div style={{
            ...
            <div style={{ paddingTop: 14, borderTop: '1px solid var(--aeva-line)' }}>
              <p style={{
                ...
                <Sparkles size={11} style={{ color: 'var(--aeva-ember)' }} /> Package includes
              </p>
          ...
          {!isExpanded && (
            <div style={{ fontSize: 12, color: 'var(--aeva-ink-soft)', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              Tap to see inclusions <ArrowRight size={11} />
            </div>
          )}
        </div>
      </div>
  ...
  const renderGrid = () => {
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
        {decorations.map(v => renderCard(v))}
      </div>
    );
  };

  const renderContinueBar = () => {
    if (isLoading || decorations.length === 0) return null;
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <span style={{ fontSize: 13.5 }}>
            Browse and pick decorations · <span style={{ color: 'var(--aeva-ink-mute)' }}>2 more steps to go</span>
          </span>
        </div>
        <button
          style={{
  ...
  return (
    <div style={{ maxWidth: 1280, margin: '0 auto', padding: '40px 32px 80px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, gap: 32, flexWrap: 'wrap' }}>
        <div>
          ...
          <p style={{ fontSize: 16, color: 'var(--aeva-ink-soft)', maxWidth: 600, lineHeight: 1.6, margin: 0 }}>
            Set the mood with curated packages and custom designs.
          </p>
        </div>

        {/* Filters */}
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          {['all', 'modern', 'classic', 'romantic'].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              style={{
      ...
      <div style={{
        ...
        <span style={{ fontSize: 12.5, color: 'var(--aeva-ink-soft)' }}>For a summer evening with</span>
        <Tag size="sm">24 guests</Tag>
        <Tag size="sm">Brooklyn loft</Tag>
        <Tag size="sm">Intimate vibe</Tag>
      </div>
