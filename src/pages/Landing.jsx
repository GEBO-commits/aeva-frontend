    ...
    <div style={{ background: 'var(--aeva-paper)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 0.9fr', gap: 56, alignItems: 'center', minHeight: 560 }}>
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '6px 12px', borderRadius: 999, background: 'var(--aeva-paper-warm)', border: '1px solid var(--aeva-line)', fontSize: 12, color: 'var(--aeva-ink-soft)', marginBottom: 32 }}>
              <span style={{ width: 6, height: 6, borderRadius: '50%', background: 'var(--aeva-ember)', animation: 'aeva-pulse 2s infinite' }}/>
            ...
            <div style={{ display: 'flex', gap: 12, marginBottom: 56 }}>
              <Button variant="ember" size="lg" onClick={() => handleNavigate('/chat')}>
                <Sparkles size={16}/> Plan with AEVA
              </Button>
              <Button variant="ghost" size="lg" onClick={() => handleNavigate('/survey')} style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ position: 'relative', height: 580 }}>
            <div style={{ position: 'absolute', top: 0, left: 30, width: 280, height: 360, borderRadius: 'var(--r-lg)', overflow: 'hidden', boxShadow: 'var(--shadow-lg)', transform: 'rotate(-3deg)', background: 'linear-gradient(135deg, var(--aeva-ember-soft) 0%, var(--aeva-sage-soft) 100%)' }}/>
            <div style={{ position: 'absolute', top: 80, right: 0, width: 240, height: 300, borderRadius: 'var(--r-lg)', overflow: 'hidden', boxShadow: 'var(--shadow-lg)', transform: 'rotate(4deg)', background: 'linear-gradient(135deg, var(--aeva-sage-soft) 0%, var(--aeva-paper-warm) 100%)' }}/>
            <div style={{ position: 'absolute', bottom: 0, left: 80, width: 220, height: 200, borderRadius: 'var(--r-lg)', overflow: 'hidden', boxShadow: 'var(--shadow-lg)', transform: 'rotate(-1deg)', background: 'linear-gradient(135deg, var(--aeva-paper-warm) 0%, var(--aeva-ember-soft) 100%)' }}/>
            {/* Floating event-card */}
            <div style={{ position: 'absolute', bottom: 30, right: 20, width: 240, padding: 16, background: 'var(--aeva-canvas)', border: '1px solid var(--aeva-line)', borderRadius: 'var(--r-md)', boxShadow: 'var(--shadow-xl)', transform: 'rotate(2deg)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                <span style={{ width: 8, height: 8, borderRadius: '50%', background: 'var(--aeva-ember)' }}/>
      ...
      <section style={{ padding: '80px 32px', maxWidth: 1280, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 20 }}>
          {[
            { kind: 'Conversation', title: 'Just talk it through', desc: 'Describe what you want in plain English. AEVA asks the right questions, drafts a plan, and lets you nudge it.', icon: 'chat', cta: 'Plan with AI', route: '/chat' },
            { kind: 'Survey', title: 'Answer 8 quick questions', desc: 'Date, vibe, headcount, budget. Two minutes, then we hand you a curated brief.', icon: 'wand', cta: 'Take the survey', route: '/survey' },
            { kind: 'Manual', title: 'Build it yourself', desc: 'You know what you want. Pick a venue, add catering, drop in vendors. We just stay out of your way.', icon: 'layers', cta: 'Open builder', route: '/plan/manual' },
          ].map((card, i) => (
            <button key={card.kind} onClick={() => handleNavigate(card.route)} style={{ textAlign: 'left', padding: 32, background: i === 0 ? 'var(--aeva-ink)' : 'var(--aeva-canvas)', color: i === 0 ? 'var(--aeva-paper)' : 'var(--aeva-ink)', border: i === 0 ? 'none' : '1px solid var(--aeva-line)', borderRadius: 'var(--r-lg)', minHeight: 280, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', cursor: 'pointer', transition: 'all 200ms' }}
              onMouseEnter={e => { e.currentTarget.style.transform = 'translateY(-3px)'; e.currentTarget.style.boxShadow = 'var(--shadow-lg)'; }}
              onMouseLeave={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = ''; }}>
              <div>
                <div style={{ width: 44, height: 44, borderRadius: 12, background: i === 0 ? 'rgba(250,248,245,0.1)' : 'var(--aeva-paper-warm)', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 24, color: i === 0 ? 'var(--aeva-ember)' : 'var(--aeva-ink)' }}>
                  <Sparkles size={20}/>
                </div>
              ...
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontSize: 13, fontWeight: 500, marginTop: 24, color: i === 0 ? 'var(--aeva-ember)' : 'var(--aeva-ink)' }}>
      ...
      <section style={{ padding: '40px 32px 80px', maxWidth: 1280, margin: '0 auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 32 }}>
          ...
          <Button variant="quiet" size="sm">Browse all <ArrowRight size={13}/></Button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr 1fr', gap: 16 }}>
          {[
            { bg: 'linear-gradient(135deg, var(--aeva-ember-soft) 0%, var(--aeva-sage-soft) 100%)', title: 'A vineyard wedding for 80', meta: 'Sonoma · Sep 2026', tone: 'Wedding · Outdoor · Intimate' },
            { bg: 'linear-gradient(135deg, var(--aeva-sage-soft) 0%, var(--aeva-paper-warm) 100%)', title: '"Future of Fintech" summit', meta: 'NYC · Apr 2026', tone: 'Corporate · 220 guests' },
            { bg: 'linear-gradient(135deg, var(--aeva-paper-warm) 0%, var(--aeva-ember-soft) 100%)', title: 'Theo turns 6', meta: 'Brooklyn · Mar 2026', tone: 'Birthday · Cozy' },
          ].map((c, i) => (
            <div key={i} style={{ position: 'relative', height: i === 0 ? 420 : 200, gridRow: i === 0 ? 'span 2' : 'auto', borderRadius: 'var(--r-lg)', overflow: 'hidden', cursor: 'pointer', background: c.bg, transition: 'transform 600ms' }}
